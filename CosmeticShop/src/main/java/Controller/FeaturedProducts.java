...
@WebServlet(name = "FeaturedProducts", urlPatterns = {"/featured-products"})
public class FeaturedProducts extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Lấy tất cả sản phẩm nổi bật (featured products - sản phẩm mới nhất)
        List<Product> allFeaturedProducts = productDB.getFeaturedProducts(1000);
        int totalProducts = allFeaturedProducts.size();
        int totalPages = (int) Math.ceil((double) totalProducts / pageSize);
        
        // Phân trang
        int startIndex = (page - 1) * pageSize;
        int endIndex = Math.min(startIndex + pageSize, totalProducts);
        List<Product> featuredProducts = allFeaturedProducts.subList(startIndex, endIndex);
        ...
        request.getRequestDispatcher("/View/featured-products.jsp").forward(request, response);
    }
